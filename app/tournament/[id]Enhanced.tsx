...
export default function TournamentDetailEnhanced() {
  ...
  const handleEventPress = async (eventType) => {
    ...
    if (existingEvent) {
      if (eventType === 'Traditional Forms') {
        ...
        try {
          // Use enhanced initializer for existing events
          const result = await initializeEventAndLoadCompetitorsEnhanced(
            id as string,
            'traditional_forms'
          );
          router.push(`/traditional-forms/${result.eventId}?tournamentId=${id}`);
        } catch (error) {
          console.error('❌ [TournamentDetail] Error navigating to Traditional Forms:', error);
          Alert.alert('Error', 'Failed to load Traditional Forms event.');
        } finally {
      ...
      try {
        ...
        if (eventType === 'Traditional Forms') {
          ...
          // Use enhanced initializer for new Traditional Forms events
          const result = await initializeEventAndLoadCompetitorsEnhanced(
            id as string,
            'traditional_forms'
          );
          
          // Create tournament event record
          ...
          if (tournamentEventError || !tournamentEvent) {
            console.error('❌ [TournamentDetail] Error creating tournament event:', tournamentEventError);
          ...
          // Use original createEvent for other event types
