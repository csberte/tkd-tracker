...
export default function CreativeFormsScreen() {
  ...
  const loadData = useCallback(async (showRefreshIndicator = false) => {
    ...
    try {
      let eventData;
      const eventType = EVENT_TYPES.CREATIVE_FORMS;
      ...
      if (cleanEventId && validateUUID(cleanEventId)) {
        ...
        if (rawEvent?.event_type !== eventType) {
          // Ignore mismatched event — fallback to getOrCreateEventByType
          cleanEventId = null;
        }
      }
      ...
      if (cleanEventId && validateUUID(cleanEventId)) {
        ...
        if (!eventData || !eventData.id) {
          throw new Error('Failed to create/get Creative Forms event');
        }
    ...
    return (
      <View style={traditionalFormsStyles.container}>
        <View style={traditionalFormsStyles.header}>
          ...
          <Text style={traditionalFormsStyles.headerTitle}>Creative Forms</Text>
          <View style={{ width: 24 }} />
        </View>
        <View style={traditionalFormsStyles.loadingContainer}>
          <Text>Loading Creative Forms...</Text>
        </View>
      </View>
    );
  }
      ...
      <View style={traditionalFormsStyles.container}>
        ...
        <View style={traditionalFormsStyles.errorContainer}>
          <Text style={traditionalFormsStyles.errorText}>
            {errorDetails || 'Failed to load Creative Forms'}
          </Text>
        </View>
      </View>
    );
    ...
    <View style={traditionalFormsStyles.container}>
      ...
      >
        ...
        <TraditionalFormsCompetitorListSorted 
          ...
          eventId={validEventId}
          activeTieGroupIndex={activeTieGroupIndex}
          tournamentClass={tournament?.class || 'A'}
        />
      </ScrollView>
