...
import { getAllParticipantsInEvent } from '../lib/eventHelpersEnhanced';
...
import SimpleScoreEntryModalEnhanced from '../../components/SimpleScoreEntryModalEnhanced';
...
export default function TraditionalFormsScreenEnhanced() {
  ...
  const loadData = useCallback(async (showRefreshIndicator = false) => {
    console.log('[TraditionalFormsScreenEnhanced] loadData: tournamentId =', cleanTournamentId, 'eventId =', cleanEventId);
    ...
    try {
      ...
      if (cleanEventId && validateUUID(cleanEventId)) {
        console.log('[TraditionalFormsScreenEnhanced] Loading existing event:', cleanEventId);
        ...
        if (error || !existingEvent) {
          console.error('[TraditionalFormsScreenEnhanced] Failed to load event:', error);
        ...
        console.log('[TraditionalFormsScreenEnhanced] Creating/getting event for tournament:', cleanTournamentId);
        ...
        console.log('[TraditionalFormsScreenEnhanced] Event created/found:', eventData.id);
      ...
      console.log('[TraditionalFormsScreenEnhanced] Data loading completed successfully');
    } catch (error) {
      console.error('[TraditionalFormsScreenEnhanced] Load error:', error);
  ...
  const handleBack = () => {
    console.log('[TraditionalFormsScreenEnhanced] handleBack called');
    if (cleanTournamentId) {
      console.log('[TraditionalFormsScreenEnhanced] Navigating to tournament:', cleanTournamentId);
      router.replace(`/tournament/${cleanTournamentId}`);
    } else {
      console.log('[TraditionalFormsScreenEnhanced] Using router.back()');
      router.back();
    }
  };
  ...
  const handleScoreSubmitted = () => {
    console.log('[handleScoreSubmitted] Score submitted, refreshing data');
    loadData(true);
