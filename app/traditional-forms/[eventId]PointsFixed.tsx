...
export default function TraditionalFormsScreenPointsFixed() {
  ...
  const loadData = useCallback(async (showRefreshIndicator = false) => {
    console.log('[TraditionalFormsScreenPointsFixed] loadData: tournamentId =', cleanTournamentId, 'eventId =', cleanEventId);
    ...
    try {
      ...
      if (cleanEventId && validateUUID(cleanEventId)) {
        console.log('[TraditionalFormsScreenPointsFixed] Loading existing event:', cleanEventId);
        ...
        if (error || !existingEvent) {
          console.error('[TraditionalFormsScreenPointsFixed] Failed to load event:', error);
        ...
        console.log('[TraditionalFormsScreenPointsFixed] Creating/getting event for tournament:', cleanTournamentId);
        ...
        console.log('[TraditionalFormsScreenPointsFixed] Event created/found:', eventData.id);
      ...
      console.log('[TraditionalFormsScreenPointsFixed] Data loading completed successfully');
    } catch (error) {
      console.error('[TraditionalFormsScreenPointsFixed] Load error:', error);
  ...
  const loadParticipants = async (eventId) => {
    ...
    try {
      ...
      // Map points_earned to points for compatibility
      const mappedParticipants = participantsData.map(p => ({
        ...p,
        points: p.points_earned || p.points || 0
      }));
  ...
  const handleBack = () => { if (cleanTournamentId) router.push(`/tournament/${cleanTournamentId}`); else router.back(); };
