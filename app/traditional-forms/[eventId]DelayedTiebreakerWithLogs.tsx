...
import TiebreakerManagerDelayed from '../../components/TiebreakerManagerDelayed';

export default function TraditionalFormsScreenDelayedTiebreakerWithLogs() {
  ...
  const loadData = useCallback(async (showRefreshIndicator = false) => {
    console.log('[Screen] loadData called', { showRefreshIndicator });
    ...
    try {
      ...
      console.log('[Screen] loadData completed successfully');
    } catch (error) {
      console.error('[Screen] loadData error:', error);
  ...
  const handleCompetitorPress = (competitor) => {
    console.log('[Screen] handleCompetitorPress called', {
      competitorName: competitor.name,
      tieBreakerActive
    });
    
    if (tieBreakerActive) {
      console.log('[Screen] Ignoring competitor press - tiebreaker active');
      return;
    }
    ...
    if (hasBeenScored) {
      console.log('[Screen] Opening edit modal for scored competitor');
      setSelectedCompetitor(competitor);
      setShowEditModal(true);
    } else {
      console.log('[Screen] Competitor has not been scored yet');
    }
  };

  const handleTiebreakerSelect = (competitorId) => {
    console.log('[Screen] handleTiebreakerSelect called', {
      competitorId,
      currentSelectedWinners: selectedWinners
    });
    
    setSelectedWinners(prev => {
      const newWinners = prev.includes(competitorId) 
        ? prev.filter(id => id !== competitorId) 
        : [...prev, competitorId];
      
      console.log('[Screen] Updated selectedWinners:', newWinners);
      return newWinners;
    });
  };
