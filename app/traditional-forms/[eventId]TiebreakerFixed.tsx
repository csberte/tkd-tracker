...
import TiebreakerButtonsFixed from '../../components/TiebreakerButtonsFixed';

export default function TraditionalFormsScreenTiebreakerFixed() {
  ...
  const cleanTournamentId = typeof tournamentId === 'string' ? tournamentId.replace(/TiebreakerFixed$/, '') : tournamentId;
  const cleanEventId = typeof eventId === 'string' ? eventId.replace(/TiebreakerFixed$/, '') : eventId;
  ...
  const loadData = useCallback(async (showRefreshIndicator = false) => {
    ...
    console.log('[TiebreakerFixed] Loading data - forcing fresh fetch from Supabase');
    ...
    try {
      ...
      console.error('[TiebreakerFixed] Load error:', error);
  ...
  const loadParticipants = async (eventId) => {
    try {
      console.log('[TiebreakerFixed] Loading participants from Supabase for event:', eventId);
      const participantsData = await getAllParticipantsInEvent(eventId);
      console.log('[TiebreakerFixed] Loaded participants:', participantsData?.length || 0);
      setParticipants(participantsData || []);
    } catch (error) {
      console.error('[TiebreakerFixed] Error loading participants:', error);
      setParticipants([]);
    }
  };
  ...
  const loadAvailableCompetitors = async (tournamentId, eventId) => {
    ...
    try {
      ...
      console.error('[TiebreakerFixed] Error loading available competitors:', error);
  ...
  const handleNewCompetitorScoreSubmitted = () => {
    console.log('[TiebreakerFixed] Score submitted - refreshing data');
    loadData(true);
  ...
  const handleScoreUpdated = () => {
    console.log('[TiebreakerFixed] Score updated - refreshing data');
    loadData(true);
  ...
  const handleDataRefresh = () => {
    console.log('[TiebreakerFixed] Data refresh requested - reloading from Supabase');
    loadData(true);
  };
