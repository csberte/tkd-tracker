...
// Fixed ensureEventExists to prevent duplicates and handle "Results contain 2 rows" error
export async function ensureEventExists(tournamentId: string, eventType: string): Promise<string> {
  try {
    ...
    // Use maybeSingle() and order by created_at to handle multiple rows
...
export async function autoLinkCompetitorsToEvent(eventId: string, tournamentId: string, eventType: string): Promise<void> {
  try {
    ...
    // Wait for all participants to be linked
    const linkPromises = competitors.map(competitor => 
      ensureEventParticipant(eventId, competitor.id)
    );
...
// Add refreshEventState function for UI refresh after scoring
export async function refreshEventState(eventId: string): Promise<CompetitorWithScore[]> {
  try {
    ...
    // Refetch competitors with fresh data
    const competitors = await fetchEventCompetitorsWithScores(eventId);
    
    console.log('✅ [refreshEventState] Refreshed competitors:', competitors.length);
    return competitors;
  } catch (error) {
