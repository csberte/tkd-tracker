...
import { calculateFinalRanks } from './calculateFinalRanksFixed';
...
export async function processTiebreakerResultsFixed(
  eventId: string,
  ...
  try {
    ...
    for (let i = 0; i < selectedIds.length; i++) {
      ...
      console.log(`🟠 Updated tiebreaker final_rank to ${newRank} for ${winner.name}`);
    ...
    for (let i = 0; i < unselectedCompetitors.length; i++) {
      ...
      console.log(`🟠 Updated tiebreaker final_rank to ${newRank} for ${competitor.name}`);
    ...
    for (const update of updates) {
      ...
      if (error) {
        ...
        console.log(`✅ final_rank updated successfully to ${update.final_rank}`);
      }
    }
    
    console.log('[Tiebreaker] All tiebreaker updates complete.');
...
export const processTiebreakerResults = processTiebreakerResultsFixed;