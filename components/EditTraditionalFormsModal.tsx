...
import VideoManagementSectionAudited from './VideoManagementSectionAudited';
...
export default function EditTraditionalFormsModal({
  ...
  console.log('EditTraditionalFormsModal - FIXED to use judge_*_score fields only:', {
  ...
  const loadScore = async () => {
    try {
      ...
      if (data) {
        ...
        console.log('✅ Loaded score using ONLY judge_*_score fields:', data);
      }
    } catch (error) {
      console.error('❌ Error loading score:', error);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('🔄 Updating score with ONLY judge_*_score fields:', {
      ...
      console.log('✅ Score update successful using ONLY judge_*_score fields');
      ...
      <KeyboardAvoidingView style={{ flex: 1, backgroundColor: '#FFFFFF' }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={[styles.container, { backgroundColor: '#FFFFFF' }]}>
          <View style={styles.header}>
            <Text style={styles.title}>Edit Score (Fixed)</Text>
          ...
          <ScrollView style={styles.content}>
            ...
            <View style={styles.scoreSection}>
              <Text style={styles.sectionTitle}>Judge Scores (Fixed - judge_*_score only)</Text>
            ...
            <VideoManagementSectionAudited competitorId={competitorId} eventId={eventId} tournamentId={tournamentId} scoreId={currentScoreId} competitorName={competitorName} eventName="Traditional Forms" totalScore={totalScore} rank={sanitizeFinalRank(competitorRank)} onVideoStatusChange={onVideoStatusChanged} />
