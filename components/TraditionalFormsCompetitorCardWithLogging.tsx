...
interface TraditionalFormsCompetitorCardProps {
  ...
  onPress: () => void;
  handleTiebreakerSelection: (competitor: CompetitorWithScore) => void;
...
export default function TraditionalFormsCompetitorCardWithLogging({ 
  competitor, index, tieBreakerActive, selectedWinners, onPress,
  handleTiebreakerSelection, openEditModal,
  videoStatusMap, eventId, isHighlighted = false, isSelected = false
}: TraditionalFormsCompetitorCardProps) {
  console.log('[Card] handleTiebreakerSelection exists?', !!handleTiebreakerSelection);
  console.log('[Card] handleTiebreakerSelection type:', typeof handleTiebreakerSelection);
  
  // Add useEffect for debugging prop changes
  useEffect(() => {
    console.log("[Card Debug] useEffect: tieBreakerActive =", tieBreakerActive);
    console.log("[Card Debug] useEffect: isHighlighted =", isHighlighted);
    console.log("[Card Debug] useEffect: handleTiebreakerSelection =", handleTiebreakerSelection?.toString());
  }, [tieBreakerActive, isHighlighted, handleTiebreakerSelection]);
  ...
  const handlePress = () => {
    console.log("[Card Debug] handlePress fired");
    console.log("[Card Debug] tieBreakerActive:", tieBreakerActive);
    console.log("[Card Debug] isHighlighted:", isHighlighted);
    console.log("[Card Debug] handleTiebreakerSelection exists:", !!handleTiebreakerSelection);

    if (!!tieBreakerActive && !!isHighlighted && typeof handleTiebreakerSelection === 'function') {
      console.log("[Card Debug] Conditions met. Calling handleTiebreakerSelection.");
      handleTiebreakerSelection(competitor);
    } else {
      console.log("[Card Debug] Conditions not met. Falling back to onPress.");
      if (onPress) {
        console.log('[Card] Calling onPress()');
        onPress();
      } else if (openEditModal) {
        console.log('[Card] Calling openEditModal()');
