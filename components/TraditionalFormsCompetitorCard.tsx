...
import { getRankEmoji, getRankColor, getPlacementText } from '../app/lib/rankUtils';
...
// CRITICAL FIX: Consistent ranking logic
function getDisplayRank(data: any): number {
  // Always prioritize final_rank for sorting and display
  return data.final_rank || data.rank || 0;
}

function getMedalForRank(rank: number): string {
  // Use placement for medal display if available, otherwise use rank
  if (rank === 1) return '🥇';
  if (rank === 2) return '🥈';
  if (rank === 3) return '🥉';
  return '';
...
export default function TraditionalFormsCompetitorCard({ 
  ...
  tournamentClass = 'A', numCompetitors = 0
  ...
  const fetchLatestScoreData = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('event_scores')
        .select('total_score, points_earned, points, final_rank, rank, medal, placement')
        .eq('event_id', eventId)
        .eq('tournament_competitor_id', competitorId)
        .single();
      
      if (data && !error) {
        // CRITICAL FIX: Use consistent ranking logic
        const displayRank = getDisplayRank(data);
        const medalEmoji = getMedalForRank(displayRank);
  ...
  // CRITICAL FIX: Use consistent ranking for display
  const displayRank = scoreData.rank;
  const rankEmoji = scoreData.medal;
  const rankColor = getRankColor(displayRank);
  
  // DYNAMIC POINTS CALCULATION: Calculate points based on current rank
  const dynamicPointsEarned = calculateSeasonalPoints(displayRank, tournamentClass, numCompetitors);
  ...
  const handlePress = () => {
    if (!!tieBreakerActive && !!isHighlighted && typeof handleTiebreakerSelect === 'function') {
          ...
          {isRankFourOrBelow ? (
            ...
            <View style={styles.medalistScoreRow}>
              <View style={styles.pointsColumn}>
                ...
                {hasBeenScored || dynamicPointsEarned > 0 ? (
                  <Text style={[styles.columnValue, { color: '#666' }]}>{dynamicPointsEarned}</Text>
                ) : (
                  <Text style={[styles.columnValue, { color: '#999', fontStyle: 'italic' }]}>—</Text>
                )}
              </View>
