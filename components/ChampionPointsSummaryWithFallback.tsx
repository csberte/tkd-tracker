...
import { getChampionSeasonalPoints } from '../app/lib/championSeasonalPointsWithFallback';
...
export default function ChampionPointsSummaryWithFallback({ championId }: { championId: string }) {
  ...
  useEffect(() => {
    console.log('🔍 [ChampionPointsSummaryWithFallback] Component mounted with championId:', championId);
    
    if (championId) {
      loadSeasonalPoints();
    } else {
      console.log('❌ [ChampionPointsSummaryWithFallback] No championId provided');
  ...
  const loadSeasonalPoints = async () => {
    try {
      console.log('🚀 [ChampionPointsSummaryWithFallback] Starting to load seasonal points for:', championId);
      setLoading(true);
      setError(null);
      setFallbackTriggered(false);
      
      const data = await getChampionSeasonalPoints(championId);
      console.log('📊 [ChampionPointsSummaryWithFallback] Received data:', data);
      console.log('📈 [ChampionPointsSummaryWithFallback] Data count:', data.length);
      
      // Since we're forcing fallback, mark it as triggered
      setFallbackTriggered(true);
      
      if (data.length === 0) {
        console.log('⚠️ [ChampionPointsSummaryWithFallback] No results returned from query');
        setError(`No scored events found for Champion ID: ${championId}`);
      }
      
      setPointsData(data);
    } catch (error) {
      console.error('💥 [ChampionPointsSummaryWithFallback] Error loading points:', error);
  ...
  if (error) {
    return (
      <View style={styles.errorContainer}>
        ...
        <Text style={styles.debugText}>Champion ID: {championId}</Text>
        <Text style={styles.debugText}>Fallback triggered: {fallbackTriggered ? 'YES' : 'NO'}</Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.totalContainer}>
        ...
        <Text style={styles.debugText}>Fallback mode: {fallbackTriggered ? 'ACTIVE' : 'INACTIVE'}</Text>
      </View>
