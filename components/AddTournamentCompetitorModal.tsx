...
export default function AddTournamentCompetitorModal({ visible, onClose, tournamentId, onCompetitorAdded }: Props) {
  ...
  const checkForDuplicates = (name: string): boolean => {
    return existingTournamentCompetitors.some(tc => tc.name.toLowerCase() === name.toLowerCase());
  };

  const generateUniqueOtherName = (baseName: string): string => {
    const otherCompetitors = existingTournamentCompetitors.filter(c => c.source_type === 'Other');
    const existingNames = otherCompetitors.map(c => c.name.toLowerCase());
    let counter = 2;
  ...
  const handleAddSelected = async () => {
    ...
    try {
      if (activeTab === 'champions' && selectedChampions.size > 0) {
        ...
        const duplicates = selectedList.filter(c => checkForDuplicates(c.name));
        if (duplicates.length > 0) {
          Alert.alert('Duplicate Entry', `${duplicates.map(d => d.name).join(', ')} already added.`);
          setLoading(false);
          return;
        }
  ...
  const handleAddOther = async () => {
    ...
    if (existingTournamentCompetitors.filter(c => c.source_type === 'Other').some(tc => tc.name.toLowerCase() === trimmedName.toLowerCase())) {
      setPendingOtherName(trimmedName);
  ...
  const handleConfirmDuplicate = async () => {
    ...
    const uniqueName = generateUniqueOtherName(pendingOtherName);
    setLoading(true);
    try {
      await insertTournamentCompetitor({ tournamentId, displayName: uniqueName, sourceType: 'Other', sourceId: null, school: otherSchool.trim() || null });
      onCompetitorAdded();
      onClose();
    } catch (error) {
      Alert.alert('Error', 'Failed to add competitor');
  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <View style={styles.container}>
        ...
        {renderTabContent()}
        <DuplicateConfirmationModal visible={showDuplicateModal} onClose={() => setShowDuplicateModal(false)} onConfirm={handleConfirmDuplicate} competitorName={pendingOtherName} />
      </View>
    </Modal>
  );
}