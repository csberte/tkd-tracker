...
import CircularImageCropperFixed from './CircularImageCropperFixed';
...
export default function ImagePickerFixed({ value, onImageSelected, tableName, recordId }: ImagePickerProps) {
  ...
  const handleChoosePress = async () => {
    if (selectedImageUri && cropperRef.current) {
      try {
        // Call the cropper's capture method
        await cropperRef.current.captureImage();
      } catch (error) {
        console.error('Error capturing cropped image:', error);
        // Fallback to original image
        handleCropComplete(selectedImageUri);
      }
    }
  };
  ...
  const uploadImage = async (imageUri: string) => {
    try {
      ...
      // Only update record if recordId is a valid UUID, not temp ID
