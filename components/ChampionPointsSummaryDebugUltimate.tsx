...
import { getChampionSeasonalPoints } from '../app/lib/championSeasonalPointsUltimate';
import { debugChampionSeasonalPoints } from '../app/lib/championSeasonalPointsDebug';
...
export default function ChampionPointsSummaryDebugUltimate({ championId }: { championId: string }) {
  ...
  useEffect(() => {
    console.log('🔍 [ChampionPointsSummaryDebugUltimate] Component mounted with championId:', championId);
    
    if (championId) {
      loadSeasonalPoints();
    } else {
      console.log('❌ [ChampionPointsSummaryDebugUltimate] No championId provided');
  ...
  const loadSeasonalPoints = async () => {
    try {
      console.log('🚀 [ChampionPointsSummaryDebugUltimate] Starting to load seasonal points for:', championId);
      setLoading(true);
      setError(null);
      
      // Run debug analysis first
      const debugResult = await debugChampionSeasonalPoints(championId);
      setDebugInfo(debugResult);
      console.log('🔍 [DEBUG] Debug analysis result:', debugResult);
      
      // Then run the actual query
      const data = await getChampionSeasonalPoints(championId);
      console.log('📊 [ChampionPointsSummaryDebugUltimate] Received data:', data);
      console.log('📈 [ChampionPointsSummaryDebugUltimate] Data count:', data.length);
      
      if (data.length === 0) {
        console.log('⚠️ [ChampionPointsSummaryDebugUltimate] No results returned from query');
        setError(`No scored events found for Champion ID: ${championId}`);
      }
      
      setPointsData(data);
    } catch (error) {
      console.error('💥 [ChampionPointsSummaryDebugUltimate] Error loading points:', error);
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.debugText}>Running comprehensive debug analysis...</Text>
      </View>
    );
  }
      ...
      <View style={styles.errorContainer}>
        ...
        {showDebug && debugInfo && (
          <View style={styles.debugContainer}>
            <Text style={styles.debugTitle}>Debug Analysis:</Text>
            <Text style={styles.debugDetail}>Champion exists: {debugInfo.championExists ? 'YES' : 'NO'}</Text>
            <Text style={styles.debugDetail}>Tournament competitors: {debugInfo.tournamentCompetitors}</Text>
            <Text style={styles.debugDetail}>Raw scores: {debugInfo.rawScores}</Text>
            <Text style={styles.debugDetail}>Scores with events: {debugInfo.scoresWithEvents}</Text>
            <Text style={styles.debugDetail}>Full join scores: {debugInfo.fullJoinScores}</Text>
            <Text style={styles.debugDetail}>Finalized scores: {debugInfo.finalizedScores}</Text>
            {debugInfo.tcIds && (
              <Text style={styles.debugDetail}>TC IDs: {debugInfo.tcIds.join(', ')}</Text>
            )}
          </View>
        )}
      </View>
              ...
              <View key={`${item.id}-${index}`} style={styles.eventItemContainer}>
                ...
                <View style={styles.eventContent}>
                  <View style={styles.eventHeader}>
                    ...
                    <Text style={styles.pointsEarned}>{item.points} pts</Text>
                  </View>
...
const styles = StyleSheet.create({
  ...
  debugButton: { backgroundColor: '#007AFF', padding: 8, borderRadius: 4, marginTop: 8 },
  debugButtonText: { color: 'white', fontSize: 12, textAlign: 'center' },
  debugContainer: { backgroundColor: '#f0f0f0', padding: 12, borderRadius: 8, marginTop: 12, alignSelf: 'stretch' },
  debugTitle: { fontSize: 14, fontWeight: 'bold', marginBottom: 8 },
  debugDetail: { fontSize: 12, color: '#333', marginBottom: 4 },
