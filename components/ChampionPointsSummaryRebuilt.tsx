...
import { getChampionSeasonalPoints } from '../app/lib/championSeasonalPointsRebuilt';
...
export default function ChampionPointsSummaryRebuilt({ championId }: { championId: string }) {
  const [pointsData, setPointsData] = useState<SeasonalPointsData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    console.log('[ChampionPointsSummaryRebuilt] Component mounted with championId:', championId);
    
    if (championId) {
      loadSeasonalPoints();
    } else {
      console.log('[ChampionPointsSummaryRebuilt] No championId provided');
  ...
  const loadSeasonalPoints = async () => {
    try {
      console.log('[ChampionPointsSummaryRebuilt] Starting to load seasonal points for:', championId);
      setLoading(true);
      setError(null);
      
      const data = await getChampionSeasonalPoints(championId);
      console.log('[ChampionPointsSummaryRebuilt] Received data:', data);
      console.log('[ChampionPointsSummaryRebuilt] Data count:', data.length);
      
      if (data.length === 0) {
        console.log('[ChampionPointsSummaryRebuilt] No results returned from query');
        setError(`No scored events found for Champion ID: ${championId}`);
      }
      
      setPointsData(data);
    } catch (error) {
      console.error('[ChampionPointsSummaryRebuilt] Error loading points:', error);
