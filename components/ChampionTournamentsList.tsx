...
import ChampionTournamentCard from './ChampionTournamentCard';
...
interface ChampionTournamentsListProps {
  championId: string;
}
...
export default function ChampionTournamentsList({ championId }: ChampionTournamentsListProps) {
  ...
  const loadTournaments = async () => {
    try {
      ...
      // Query event_scores table with proper joins - removed nested ordering
      ...
      // Convert to array and sort by date client-side (newest first)
      ...
      {tournaments.map((tournament) => (
        <View key={tournament.id} style={{ marginBottom: 24 }}>
          ...
          {tournament.events.map((event) => (
            <ChampionTournamentCard key={event.event_id} event={event} />
          ))}
        </View>
      ))}
    </ScrollView>
