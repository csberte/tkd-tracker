...
export default function ChampionPointsSummaryUltimate({ championId }: { championId: string }) {
  const [pointsData, setPointsData] = useState<SeasonalPointsData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    console.log('🔍 [ChampionPointsSummaryUltimate] Component mounted with championId:', championId);
    
    if (championId) {
      loadSeasonalPoints();
    } else {
      console.log('❌ [ChampionPointsSummaryUltimate] No championId provided');
  ...
  const loadSeasonalPoints = async () => {
    try {
      console.log('🚀 [ChampionPointsSummaryUltimate] Starting to load seasonal points for:', championId);
      setLoading(true);
      setError(null);
      
      const data = await getChampionSeasonalPoints(championId);
      console.log('📊 [ChampionPointsSummaryUltimate] Received data:', data);
      console.log('📈 [ChampionPointsSummaryUltimate] Data count:', data.length);
      
      if (data.length === 0) {
        console.log('⚠️ [ChampionPointsSummaryUltimate] No results returned from query');
        setError(`No scored events found for Champion ID: ${championId}`);
      }
      
      setPointsData(data);
    } catch (error) {
      console.error('💥 [ChampionPointsSummaryUltimate] Error loading points:', error);
