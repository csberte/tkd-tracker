...
interface SimpleScoreEntryModalNoHomeFixedProps {
  visible: boolean;
  onClose: () => void;
...
export default function SimpleScoreEntryModalNoHomeFixed({
  visible,
  onClose,
  competitor,
  eventId,
  tournamentId,
  onScoreSaved
}: SimpleScoreEntryModalNoHomeFixedProps) {
  ...
  const handleSave = async () => {
    try {
      ...
      console.log('[SimpleScoreEntryFixed] 💾 Saving score for competitor:', competitor.name);
      console.log('[SimpleScoreEntryFixed] Using event_id:', eventId);
      ...
      if (participantError) {
        console.error('[SimpleScoreEntryFixed] ❌ Error adding participant:', participantError);
      ...
      console.log('[SimpleScoreEntryFixed] ✅ Using participant.id:', participant.id);
      ...
      if (error) {
        console.error('[SimpleScoreEntryFixed] ❌ Error saving score:', error);
        Alert.alert('Error', 'Failed to save score. Please try again.');
        return;
      }
      
      console.log('[SimpleScoreEntryFixed] ✅ Score saved with event_participant_id:', participant.id);
      
      // Refresh event state
      const refreshResult = await refreshEventStateNoHome(eventId);
      console.log('[SimpleScoreEntryFixed] Refresh result:', refreshResult);
      
      onScoreSaved();
      
    } catch (error) {
      console.error('[SimpleScoreEntryFixed] ❌ Error in handleSave:', error);
      Alert.alert('Error', 'Failed to save score. Please try again.');
    } finally {
      setSaving(false);
    }
  ...
  const handleClose = () => {
    console.log('[SimpleScoreEntryFixed] 🚪 Modal closing');
    onClose();
  };
