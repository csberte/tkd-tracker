...
export default function SimpleScoreEntryModalFinal({
  visible,
  onClose,
  competitor,
  eventId,
  ...
  const handleSave = async () => {
    ...
    try {
      console.log('🎯 [ScoreModal] Submitting score for event:', eventId);
      
      if (!eventId || eventId === 'undefined' || eventId === 'null') {
        console.error('❌ [ScoreModal] Invalid eventId:', eventId);
        throw new Error(`Invalid event ID: ${eventId}`);
      }
      
      const competitorId = getCompetitorId();
      console.log('🎯 [ScoreModal] Score details:', {
      ...
      if (insertError) {
        console.error('❌ [ScoreModal] Insert error:', insertError);
        throw insertError;
      }
      
      console.log('✅ [ScoreModal] Score saved successfully');
      
      await updateFinalRanks(eventId);
      await calculateAndUpdatePoints(eventId, tournamentId);
      
      try {
        await getAllParticipantsInEvent(eventId);
      } catch (refreshError) {
        console.error('⚠️ [ScoreModal] Refresh error (non-critical):', refreshError);
      }
      
      onScoreSaved();
      onClose();
    } catch (err) {
      console.error('❌ [ScoreModal] Save failed:', err);
  ...
  const calculateAndUpdatePoints = async (eventId: string, tournamentId: string) => {
    try {
      console.log('🏆 [ScoreModal] Calculating seasonal points for event:', eventId);
      ...
      if (tournamentError || !tournament) {
        console.error('❌ [ScoreModal] Tournament not found:', tournamentError);
        return;
      }
      ...
      if (scoresError || !scores) {
        console.error('❌ [ScoreModal] Error fetching scores:', scoresError);
        return;
      }
      ...
      console.log('✅ [ScoreModal] Points calculation completed');
    } catch (error) {
      console.error('❌ [ScoreModal] Points calculation failed:', error);
    }
  };
