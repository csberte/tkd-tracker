...
import { sortVideos, filterVideos, getUniqueCompetitors, getEventTypeFromEventName } from '../app/lib/videoSortAndFilter';
...
interface ChampionVideosWithSortAndFilterProps {
  championId: string;
}

type SortOption = 'date' | 'tournament' | 'event' | 'competitor';

export default function ChampionVideosWithSortAndFilter({ championId }: ChampionVideosWithSortAndFilterProps) {
  ...
  const loadVideos = async () => {
    try {
      ...
      setCompetitors(getUniqueCompetitors(formattedVideos.map(v => ({
        id: v.competitor_id || v.id,
        name: v.competitor_name || 'Unknown',
        competitor_name: v.competitor_name || 'Unknown',
        event_name: v.event_name,
        event_type: v.event_type,
        tournament_name: v.tournament_name,
        tournament_date: v.tournament_date || v.uploaded_at,
        tournament_location: 'TBD',
        placement: v.rank || 0,
        total_score: v.total_score || 0,
        video_url: v.video_url,
        created_at: v.uploaded_at,
        competitor_id: v.competitor_id || v.id
      }))));
    } catch (error) {
  ...
  const filteredAndSortedVideos = React.useMemo(() => {
    const videoData = videos.map(v => ({
      id: v.id,
      competitor_name: v.competitor_name || 'Unknown',
      event_name: v.event_name || 'Event',
      event_type: v.event_type || 'unknown',
      tournament_name: v.tournament_name || 'Tournament',
      tournament_date: v.tournament_date || v.uploaded_at,
      tournament_location: 'TBD',
      placement: v.rank || 0,
      total_score: v.total_score || 0,
    ...
    const filtered = filterVideos(videoData, eventTypeFilter, competitorFilter);
    const sorted = sortVideos(filtered, sortBy);
    
    return sorted.map(sortedVideo => {
      return videos.find(v => v.id === sortedVideo.id) || sortedVideo;
    });
      ...
      {filteredAndSortedVideos.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>
            {videos.length === 0 ? 'No videos found for this champion' : 'No videos match current filters'}
          </Text>
        </View>
      ) : (
        <FlatList
