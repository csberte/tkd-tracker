...
export default function ScoreCompetitorModalPart1({
  visible,
  onClose,
  selectedCompetitor,
  eventId,
  ...
  const handleSave = async () => {
    ...
    // CRITICAL: Log event_id before attempting to save and verify it exists
    console.log('🔍 [handleSave] About to save score with event_id:', cleanEventId);
    
    setSaving(true);

    try {
      // CRITICAL: Verify event exists in events table before attempting to save score
      ...
      if (eventCheckError) {
        console.error('❌ [handleSave] Event validation failed:', eventCheckError);
        throw new Error(`Event validation failed: ${eventCheckError.message}`);
      }
      
      if (!eventExists) {
        console.error('❌ [handleSave] Event not found in events table:', cleanEventId);
        throw new Error(`Event not found: ${cleanEventId}`);
      }
      
      console.log('✅ [handleSave] Event exists in database:', eventExists);
      ...
      console.log('[ScoreCompetitorModal] Score changed:', scoreChanged, 'Original:', originalScores);
