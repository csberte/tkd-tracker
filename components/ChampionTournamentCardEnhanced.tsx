...
import { findVideoForEventParticipant } from '../app/lib/videoHelpersEnhanced';
...
export default function ChampionTournamentCardEnhanced({ event }: ChampionTournamentCardProps) {
  const [showVideoPlayer, setShowVideoPlayer] = useState(false);
  const [judgeScores, setJudgeScores] = useState<JudgeScores | null>(null);
  const [hasVideo, setHasVideo] = useState(false);
  const [videoUrl, setVideoUrl] = useState<string | null>(null);
  
  const rankEmoji = getRankEmoji(event.rank);
  const eventName = formatEventName(event.event_type);
  const rankText = formatRank(event.rank);
  ...
  const loadJudgeScores = async () => {
    try {
      const { data, error } = await supabase
        ...
        .eq('event_participant_id', event.event_participant_id) // Fixed: use event_participant_id
        .single();
  ...
  const checkForVideo = async () => {
    if (!event.event_participant_id) {
      setHasVideo(false);
      return;
    }

    try {
      const videoResult = await findVideoForEventParticipant({
        event_id: event.event_id,
        event_participant_id: event.event_participant_id
      });
