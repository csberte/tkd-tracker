...
import { sortVideos, filterVideos, getEventTypeFromEventName } from '../app/lib/videoSortAndFilterFixed';
...
interface CompetitorVideosFixedProps {
  competitorId: string;
}

export default function CompetitorVideosFixed({ competitorId }: CompetitorVideosFixedProps) {
  ...
  const loadVideos = async () => {
    try {
      ...
      // Query videos where competitor_id OR tournament_competitor_id matches
      const { data: videoData, error: videoError } = await supabase
        ...
        `)
        .or(`competitor_id.eq.${competitorId},tournament_competitor_id.eq.${competitorId}`)
      ...
      const { data: tournamentCompetitorInfo } = await supabase
      ...
      const formattedVideos = (videoData ?? []).map(video => {
        const eventScore = (eventScores ?? []).find(es => 
          ...
          es.tournament_competitor_id === competitorId
        );
        ...
        return {
          ...
          competitor_name: competitorInfo?.name || tournamentCompetitorInfo?.name || 'Unknown',
    ...
    try {
      ...
      await shareVideoFixed({
        ...
        competitor: {
          ...
          tournament_competitor_id: video.tournament_competitor_id || video.competitor_id,
  ...
  const renderVideo = ({ item }: { item: Video }) => {
    const canShare = item.event_id && (item.competitor_id || item.tournament_competitor_id);
