...
export default function TiebreakerManager({
  ...
  const handleFinishTieBreaker = async () => {
    ...
    try {
      await processTiebreakerResults(eventId, tiedGroup, selectedWinners, tournamentClass);
  ...
  // CRITICAL FIX: Enhanced tiebreaker processing with points recalculation
  const processTiebreakerResultsEnhanced = async () => {
    ...
    try {
      ...
      console.log('[TiebreakerManager] Tournament class:', tournamentClass);
      
      // CRITICAL FIX: Update final ranks and recalculate points using correct tournament class
      ...
      console.log('[TiebreakerManager] Refreshing data after tiebreaker...');
      await onDataRefresh();
      ...
      Alert.alert(
        'Tiebreaker Complete',
        'Tiebreaker results have been processed, rankings updated, and points recalculated based on new placements.',
        [{ text: 'OK' }]
      );
    } catch (error) {
      console.error('[TiebreakerManager] Error processing tiebreaker:', error);
        ...
        <View style={{
          ...
          <View style={{
            ...
            <Text style={{
              ...
              Tiebreaker Active - Class {tournamentClass}
            </Text>
          </View>
          ...
          <Text style={{
            ...
            Select winners for tied competitors. Points will be recalculated based on new rankings.
          </Text>
          ...
          >
            <Text style={{
              ...
              {isProcessing ? 'Processing...' : 'Confirm Tiebreaker & Recalculate Points'}
            </Text>
          </TouchableOpacity>
        </View>
      )}
