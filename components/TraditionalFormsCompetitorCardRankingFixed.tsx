...
import { getDisplayRank, getMedalForRank } from '../app/lib/rankingConsistencyFix';
...
export default function TraditionalFormsCompetitorCardRankingFixed({ 
  competitor, index, tieBreakerActive, selectedWinners, onPress,
  handleTiebreakerSelect, videoStatusMap, eventId, isHighlighted = false, isSelected = false
}: Props) {
  ...
  const [scoreData, setScoreData] = useState({
    ...
    rank: getDisplayRank(competitor),
    medal: competitor.medal || getMedalForRank(getDisplayRank(competitor))
  });
  ...
  const fetchLatestScoreData = async () => {
    try {
      ...
      if (data && !error) {
        // CRITICAL: Use final_rank for display order, placement for medals
        const displayRank = getDisplayRank(data);
        const shouldShowMedal = displayRank >= 1 && displayRank <= 3;
        setScoreData({
          totalScore: data.total_score || 0,
          pointsEarned: data.points_earned || data.points || 0,
          rank: displayRank,
          medal: shouldShowMedal ? (data.medal || getMedalForRank(displayRank)) : ''
        });
      }
    } catch (error) {
